...
ServiceSDBus::ServiceSDBus(QObject *parent) : ServiceBase(parent)
    , m_bus(nullptr)
{
...
void ServiceSDBus::InitService()
{
    qInfo() << "[ServiceSDBus]init service:" << libPath();
...
void ServiceSDBus::InitThread()
{
    ...
    if (sd_bus_request_name(m_bus, ServiceBase::name().toStdString().c_str(), 0) < 0) {
...
//                                    ServiceBase::interface().toStdString().c_str(), // TODO:interface问题
//                                    (const sd_bus_vtable *)obj,
//                                    NULL) < 0) {
...
bool ServiceSDBus::Register()
{
    ...
    if (!objFunc) {
        qWarning() << "[ServiceSDBus]failed to resolve the `DSMRegisterObject` method: "<< fileInfo.fileName() ;
        if (lib->isLoaded())
            lib->unload();
        lib->deleteLater();
        return false;
    ...
    int ret = objFunc(ServiceBase::name().toStdString().c_str(), (void*)m_bus); // TODO:old type cast
    if (ret) {
        return false;
    }
