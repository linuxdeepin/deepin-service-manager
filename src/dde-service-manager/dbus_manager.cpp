...
DBusManager::DBusManager(SessionType type, QObject *parent)
    : QObject(parent)
    , m_type(type)
{
    Q_ASSERT(m_type == SessionType::Session || m_type == SessionType::System);

    // 启动时加载所有插件
    QDir libDir(getPluginPath(m_type));
...
DBusManager *DBusManager::instance(SessionType type, QObject *parent)
{
    if (!m_instance)
        m_instance = new DBusManager(type, parent);
    return m_instance;
}
...
{
    QFileInfo fileInfo(getPluginPath(m_type) + QDir::separator() + fileName);
    ...
    if (info.name.isEmpty()) {
        qCDebug(plugin) << "plugin name is empty.";
        if (lib->isLoaded())
            lib->unload();
        lib->deleteLater();
        return;
    ...
    auto initPlugin = [ = ] (const QString &fileName, const QString &name, QLibrary *lib) {
        ...
        qCDebug(plugin) << "load: " << name;
        startFun();
        qCDebug(plugin) << "load finished: " << name;
    ...
    if (info.session_type == SessionType::System&& !info.service.isEmpty()) {
        if (QDBusConnection::systemBus().interface()->isServiceRegistered(info.service)) {
            initPlugin(fileInfo.fileName(), info.name, lib);
        } else {
            qCDebug(plugin) << "system service " << info.service << " not start, wait...";
            QDBusServiceWatcher *watcher = new QDBusServiceWatcher(info.service, QDBusConnection::systemBus(), QDBusServiceWatcher::WatchForOwnerChange, this);
            connect(watcher, &QDBusServiceWatcher::serviceRegistered, this, [ = ] {
                qCDebug(plugin) << "system service " << info.service << " started";
                initPlugin(fileInfo.fileName(), info.name, lib);
                watcher->deleteLater();
            });
        }
    } else if(info.session_type == SessionType::Session && !info.service.isEmpty()) {
 ...
 * @return
 */
const QString DBusManager::getPluginPath(SessionType type)
{
#ifdef QT_DEBUG
    ...
    if (type == Session) {
        ...
        qCCritical(plugin) << "unknown type";
        return QString();
    }
#else
    QString path;
    if (type == Session) {
        path = "/usr/lib/dde-plugins/session";
    } else if(type == System) {
        path = "/usr/lib/dde-plugins/system";
    } else {
        qCCritical(plugin) << "unknown type";
        return path;
    }
 ...
 */
void DBusManager::resolveInfo(const char *data, DBusManager::PluginInfo &info)
{
    QJsonParseError err;
    ...
    info.session_type = obj.value("session_type").toInt();
